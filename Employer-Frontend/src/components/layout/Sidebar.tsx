...
const main: NavEntry[] = [{ to: '/dashboard', label: 'Dashboard', icon: LayoutDashboard }]

const recruitment: NavEntry[] = [
...
function NavItem({ to, label, icon: Icon, badge, collapsed }: NavEntry & { collapsed: boolean }) {
  return (
    <NavLink
      to={to}
      className={({ isActive }) =>
...
function Group({ label, items, collapsed }: { label?: string; items: NavEntry[]; collapsed: boolean }) {
  return (
