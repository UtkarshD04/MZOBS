...
export default function TagInput({ value, onChange, placeholder }: { value: string[]; onChange: (tags: string[]) => void; placeholder?: string }) {
  ...
  return (
    <div className={cn(inputClass, 'h-auto min-h-10 py-2 flex flex-wrap gap-1.5 items-center cursor-text')} onClick={(e) => (e.currentTarget.querySelector('input') as HTMLInputElement | null)?.focus()}>
      {value.map((tag) => (
