...
import { Upload, FileText, Download, UserPlus, Inbox, ShieldCheck } from 'lucide-react'
    ...
    <StaggerGroup>
      <StaggerItem className="flex items-start justify-between gap-5 flex-wrap mb-6">
        <div>
          ...
          <p className="text-sm text-ink-secondary mt-1">Bulk-intake resumes and allocate them to HR to verify.</p>
        </div>
        {isAdmin && (
      ...
      {isAdmin && stats?.perStaff?.length > 0 && (
      ...
      <StaggerItem>
        {rows.length === 0 ? (
          <Card>
            <EmptyState icon={Inbox} tone="navy" title="No resumes here" body="Bulk-upload resumes to start allocating them to HR." />
          </Card>
        ) : (
          <Card>
            <TableWrap className="border-none rounded-none">
              <Table columns={isAdmin ? ['', 'File', 'Status', 'Assigned to', 'Uploaded', ''] : ['File', 'Status', 'Uploaded', '']}>
                {rows.map((r) => (
                  <Tr key={r.id}>
                    {isAdmin && (
                      <Td>
