...
import { useEmployerRevenueTrendQuery } from '../../hooks/usePayments'
...
export default function EmployerRevenueTrend() {
  ...
  const { data, isLoading, isError, refetch } = useEmployerRevenueTrendQuery(params)
  ...
  return (
    <StaggerGroup>
      <StaggerItem className="flex items-start justify-between gap-5 flex-wrap mb-4">
        <div>
          <h1 className="text-2xl font-bold tracking-tight">Employer Revenue</h1>
          <p className="text-sm text-ink-secondary mt-1">Job posting fee revenue — pick a preset or any custom number of days.</p>
        </div>
      ...
      <StaggerItem className="grid grid-cols-2 gap-4 mb-5">
        <Card hover pad>
          <div className="flex items-center justify-between">
            <span className="text-xs font-semibold tracking-wide uppercase text-ink-tertiary">Invoices in range</span>
      ...
      <StaggerItem>
        <Card>
          <CardHead>
            <span className="text-[15px] font-semibold">Paid invoices</span>
            <span className="text-xs text-ink-tertiary">{activeLabel}</span>
          </CardHead>
          <div className="p-[22px] pt-6">
            {chartData.every((d) => d.value === 0) ? (
              <p className="text-[13px] text-ink-secondary">No paid invoices in this period yet.</p>
            ) : (
              <AreaLineChart data={chartData} formatValue={(v) => `${v} invoice${v === 1 ? '' : 's'}`} />
            )}
          </div>
        </Card>
      </StaggerItem>
