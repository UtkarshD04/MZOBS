...
function CreateCompanyModal({ app, onDone }) {
  ...
  if (result) {
    return (
      <>
        <ModalHead title="Company onboarded" onClose={app.closeModal} />
        <ModalBody>
          <p className="text-[13px] text-ink-secondary mb-4">
            <b className="text-ink">{result.company.name}</b> has been created with <b className="text-ink">{result.user.name}</b> as Admin. There's no
            automated invite email yet — share this temporary password with them directly.
          </p>
  ...
  return (
    <>
      <ModalHead title="Onboard a company" onClose={app.closeModal} />
      <ModalBody>
        <Field label="Company name">
          <Input value={companyName} onChange={(e) => setCompanyName(e.target.value)} />
        </Field>
        <Field label="Admin contact name" hint="The rest of the company profile — industry, size, GSTIN — gets filled in later from their Company Profile page.">
      ...
      <ModalFoot>
        ...
        <Button variant="primary" onClick={submit} disabled={create.isPending || !companyName || !adminName || !adminEmail}>
  ...
  return (
    <StaggerGroup>
      <StaggerItem className="flex items-start justify-between gap-5 flex-wrap mb-6">
        ...
        <Button variant="primary" onClick={() => app.openModal(<CreateCompanyModal app={app} onDone={refetch} />)}>
