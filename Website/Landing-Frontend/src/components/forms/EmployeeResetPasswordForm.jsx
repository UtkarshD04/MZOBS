...
import { resetPasswordEmployee } from '../../lib/employeeAuth'
...
export default function EmployeeResetPasswordForm() {
  ...
  async function handleSubmit(e) {
    ...
    try {
      await resetPasswordEmployee({ token, password: form.password })
      setStatus('success')
    } catch (err) {
      setStatus('idle')
  ...
  if (!token) {
    return (
      <div className="text-center py-6">
        ...
        <Link to="/employees/forgot-password" className="inline-block mt-4 text-xs font-bold text-[#595959] hover:text-black transition-colors">
          Request a new link
        </Link>
      </div>
    )
