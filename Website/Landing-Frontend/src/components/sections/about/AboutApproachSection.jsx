          ...
          {APPROACH_DATA.steps.map((step) => {
            ...
            return (
              <StaggerItem key={step.num}>
                <TiltCard maxTilt={3} y={-6} className="bg-[#F5F5F5] rounded-3xl p-6 h-full border border-[#e0e0e0] hover:shadow-lg hover:bg-white">
                  <span className="text-[13px] font-black text-[var(--careers-accent)]">{step.num}</span>
                  <div className="w-11 h-11 rounded-xl bg-white flex items-center justify-center my-4 transition-transform duration-300 group-hover:scale-110">
