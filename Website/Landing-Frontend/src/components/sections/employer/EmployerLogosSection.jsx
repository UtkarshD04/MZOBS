...
export default function EmployerLogosSection() {
  return (
    <section id="companies" className="bg-[#f7f9fb] py-16 md:py-20 px-6 md:px-12">
        ...
        <FadeInView delay={0.1} className="mt-10 flex flex-wrap items-center justify-center gap-x-10 gap-y-8 sm:gap-x-14">
          {TRUSTED_LOGOS_DATA.logos.map((logo) => (
            <div key={logo.name} className="group relative">
              <div className="absolute -top-8 left-1/2 -translate-x-1/2 whitespace-nowrap px-2.5 py-1 rounded-md bg-[#102a43] text-white text-[11px] font-semibold opacity-0 scale-95 group-hover:opacity-100 group-hover:scale-100 transition-all duration-200 pointer-events-none">
                {logo.name}
              </div>
              <img
                src={logo.logo}
