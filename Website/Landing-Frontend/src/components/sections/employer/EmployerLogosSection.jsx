...
export default function EmployerLogosSection() {
  return (
    <section className="bg-(--jobs-blue-tint) py-12 md:py-16 px-6 md:px-12 overflow-hidden">
            ...
            return (
              ...
              >
                <div className="group relative">
                  <div className="absolute -top-9 left-1/2 -translate-x-1/2 whitespace-nowrap px-2.5 py-1 rounded-full bg-(--jobs-teal-dark) text-white text-[11px] font-bold opacity-0 scale-95 group-hover:opacity-100 group-hover:scale-100 transition-all duration-200 pointer-events-none">
                    {logo.name}
                    <span className="absolute top-full left-1/2 -translate-x-1/2 border-4 border-transparent border-t-(--jobs-teal-dark)" />
                  </div>

                  <div
                    style={{ width: v.size, height: v.size }}
                    className="rounded-full bg-white border border-(--jobs-border) shadow-sm flex items-center justify-center p-4 transition-all duration-300 hover:shadow-xl hover:scale-110 hover:border-(--jobs-teal-dark)/30 cursor-default"
                  >
