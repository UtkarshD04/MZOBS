...
function CandidateBoard() {
  return <div className="relative mx-auto max-w-[510px] lg:ml-auto">
    <div data-hero-glow className="absolute -top-8 -right-6 h-36 w-36 rounded-full bg-[#0a6f64]/25 opacity-90 blur-[1px]" />
...
export default function EmployerHero() {
  ...
  return (
    <section ref={heroRef} id="home" className="relative overflow-hidden bg-[#f7f9fb] px-6 pb-20 pt-32 md:px-12 md:pb-28 md:pt-40">
      <div aria-hidden="true" className="absolute inset-x-0 top-0 h-1 bg-[#0a6f64]" />
      <div data-hero-orbit aria-hidden="true" className="absolute left-[45%] top-20 h-[550px] w-[550px] rounded-full border border-[#102a43]/10" />
      <div className="relative mx-auto grid max-w-7xl items-center gap-14 lg:grid-cols-[1.04fr_.96fr] lg:gap-20">
        <div>
          <FadeInLoad delay={0.08}>
            <h1 className="mt-6 max-w-3xl font-sans text-[48px] font-bold leading-[0.96] tracking-[-0.045em] text-[#102a43] sm:text-6xl md:text-[76px]">Great teams start with a better first <em className="font-sans font-normal text-[#0a6f64]">conversation.</em></h1>
          </FadeInLoad>

          <FadeInLoad delay={0.16}>
            <p className="mt-7 max-w-xl text-base leading-relaxed text-[#51697e] sm:text-lg">Mzobs brings verified candidates and serious employers into one clear hiring flow—so you can spend less time sorting and more time meeting the right people.</p>
          </FadeInLoad>
          ...
          <FadeInLoad delay={0.22}>
            <div className="mt-9 flex flex-wrap items-center gap-4">
              <Link
                to="/employers/signup"
                className="inline-flex min-h-12 items-center gap-2 rounded-full bg-[#102a43] px-6 text-sm font-bold text-[#e8f8f5] transition-transform duration-200 hover:-translate-y-1"
              >Start hiring <ArrowUpRight size={17} />
              </Link>
              <a
                href="#how-it-works" className="inline-flex min-h-12 items-center text-sm font-bold text-[#102a43] underline decoration-[#0a6f64] decoration-2 underline-offset-6 hover:text-[#0a6f64]">See the process</a>
            </div>
          </FadeInLoad>
        </div>
