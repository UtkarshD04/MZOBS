  ...
  return (
    ...
    >
              ...
              {LINKS.map((link, i) => (
                ...
                >
                  <Link
                    ...
                    className="block px-3 py-2 rounded-lg text-[12px] font-bold text-(--jobs-ink-soft) hover:bg-(--jobs-teal-tint) hover:text-(--jobs-teal-dark) hover:translate-x-1 transition-all"
                  >
                    {link.label}
                  </Link>
                </motion.li>
      ...
      <div className="relative">
        {!open && (
          <span className="absolute inset-0 rounded-full bg-(--jobs-teal-dark)/50 animate-ping pointer-events-none" />
        )}
        <motion.button
          type="button"
          onClick={() => setOpen((v) => !v)}
          ...
          className="relative w-12 h-12 rounded-full bg-(--jobs-teal-dark) text-white shadow-lg flex items-center justify-center hover:bg-(--jobs-navy) transition-colors"
        >
