...
// own --explorer-* palette — deep teal for key actions (Find jobs, View
// role, Apply now, Post a requirement), white/navy/bordered for everything
// less important. Polymorphic the same way the rest of this codebase's link-
// like controls are: pass `to` for a react-router route, `href` for a plain
// link, or neither for a real `<button>`.
...
const VARIANT_CLASSES = {
  primary:
    'explorer-btn-primary bg-(--explorer-teal) text-white shadow-[0_1px_2px_rgba(11,122,109,0.16),0_10px_20px_-10px_rgba(11,122,109,0.55)] hover:bg-(--explorer-teal-hover) hover:shadow-[0_1px_2px_rgba(11,122,109,0.2),0_14px_26px_-10px_rgba(7,95,85,0.55)]',
  secondary:
    'bg-white text-(--explorer-navy) border border-(--explorer-border) hover:bg-(--explorer-teal-surface) hover:border-(--explorer-teal-border) hover:text-(--explorer-teal)',
}
...
const BASE_CLASSES =
  ...
  'focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-(--explorer-teal) ' +
...
// Plain-text action (View all jobs, View company jobs...) — deep teal text,
...
export function ExplorerTextLink({ className = '', children, to, href, arrow = true, ...props }) {
  const classes = `group/link relative inline-flex items-center gap-1 font-bold text-(--explorer-teal) hover:text-(--explorer-teal-hover) transition-colors duration-150 rounded-xs focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-(--explorer-teal) ${className}`
  const arrowEl = arrow ? (
    <ArrowRight
      size={14}
      aria-hidden="true"
