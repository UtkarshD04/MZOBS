...
import { Menu as MenuIcon, Search, Sun, Moon, Bell, ChevronDown, Building2, Settings, LifeBuoy, Plus, ShieldCheck, MailOpen, LogOut } from 'lucide-react'
...
export default function TopNavbar() {
  ...
  return (
    <header className="fixed top-0 left-0 right-0 h-16 z-[60] flex items-center gap-4 px-5 bg-surface border-b border-border">
      ...
      <div className="flex items-center gap-[9px] font-bold text-base tracking-tight w-52 flex-shrink-0 max-lg:w-auto">
      ...
      >
        <Search size={16} />
        <span className="text-[13px] flex-1 text-left max-sm:hidden">Search resumes or jobs</span>
      ...
      <div className="flex items-center gap-1.5 ml-auto">
        ...
        <Popover className="relative">
          <PopoverButton title="Notifications" className="relative w-9 h-9 rounded-[10px] flex items-center justify-center text-ink-secondary hover:bg-surface-hover hover:text-ink transition-colors">
            <Bell size={18} />
            {unread > 0 && <span className="notif-dot absolute top-1.5 right-1.5 w-2 h-2 rounded-full bg-amber-dot border-2 border-surface" />}
          </PopoverButton>
          <PopoverPanel
            transition
            anchor="bottom end"
        ...
        <Popover className="relative">
          <PopoverButton className="flex items-center gap-2 pl-1.5 pr-2 h-9 rounded-[10px] cursor-pointer hover:bg-surface-hover">
            ...
            <span className="text-[13px] font-semibold max-w-[130px] truncate max-sm:hidden">{company?.name}</span>
            <ChevronDown size={13} className="text-ink-tertiary max-sm:hidden" />
          </PopoverButton>
          <PopoverPanel
            transition
            anchor="bottom end"
            ...
            {({ close }) => (
              <>
                ...
                <div className="p-1.5">
                  ...
                  <button onClick={toggleTheme} className="flex w-full items-center gap-2.5 px-2.5 py-[9px] rounded-lg cursor-pointer hover:bg-surface-hover text-[13px]">
