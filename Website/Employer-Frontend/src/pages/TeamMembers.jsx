...
import { inviteSchema } from '../schemas/teamSchema'
...
export default function TeamMembers() {
  ...
  const {
    ...
    reset,
    formState: { errors },
  } = useForm({ resolver: zodResolver(inviteSchema), defaultValues: { name: '', email: '', role: 'Recruiter' } })
          ...
          <Table columns={['Member', 'Role', 'Status', 'Last Active', '']}>
            {team.map((m) => (
              <Tr key={m.id}>
                ...
                <Td>
                  <Select value={m.role} onChange={(e) => updateRole.mutate({ id: m.id, role: e.target.value })} className="!h-8 !text-[12.5px] w-40">
