...
import {
  ...
  useAssignSkillTrackMutation,
  useSetTrustScoreMutation,
...
function TrustScoreModal({ app, employee, onDone }) {
  const [score, setScore] = useState(employee.shortlist?.trustScore ?? 75)
  const [note, setNote] = useState('')
  const setTrustScore = useSetTrustScoreMutation()

  function submit() {
    setTrustScore.mutate(
      { employeeId: employee.id, score, note },
      {
        onSuccess: () => {
          app.closeModal()
          app.addToast('success', `${employee.name} scored ${score}/100 and shortlisted`)
          onDone?.()
        },
        onError: (err) => app.addToast('error', err.response?.data?.message ?? 'Something went wrong'),
      }
  ...
  return (
    <>
      <ModalHead title={`Trust score — ${employee.name}`} onClose={app.closeModal} />
      <ModalBody>
        <Field label={`Trust score (${score}/100)`} hint="Saving this shortlists the candidate.">
          <input type="range" min={0} max={100} value={score} onChange={(e) => setScore(Number(e.target.value))} className="w-full accent-navy" />
        </Field>
        <Field label="Note" optional>
          <Textarea value={note} onChange={(e) => setNote(e.target.value)} placeholder="Why this score?" />
        </Field>
      </ModalBody>
      <ModalFoot>
        <Button onClick={app.closeModal} disabled={setTrustScore.isPending}>
          Cancel
        </Button>
        <Button variant="primary" onClick={submit} disabled={setTrustScore.isPending}>
          {setTrustScore.isPending ? 'Saving...' : 'Score & shortlist'}
        </Button>
      </ModalFoot>
    </>
  )
        ...
        <Card>
          ...
          {completed.length === 0 ? (
            ...
            <TableWrap className="border-none rounded-none">
              <Table columns={['Candidate', 'Panel', 'Communication', 'Domain', 'Overall', 'Track assigned', 'Trust score']}>
                {completed.map((m) => {
                  ...
                  return (
                    <Tr key={m.id}>
                      ...
                      <Td>
                        {emp?.shortlist?.trustScore != null ? (
                          <Badge tone="green" dot={false}>
                            Shortlisted · {emp.shortlist.trustScore}/100
                          </Badge>
                        ) : emp ? (
                          <Button size="sm" onClick={() => app.openModal(<TrustScoreModal app={app} employee={emp} onDone={refetchAll} />)}>
                            Score & shortlist
                          </Button>
                        ) : (
                          <span className="text-ink-tertiary text-[12.5px]">—</span>
                        )}
                      </Td>
                    </Tr>
                  )
