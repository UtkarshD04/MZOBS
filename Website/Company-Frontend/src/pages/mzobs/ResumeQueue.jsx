...
function AssignResumeModal({ app, employeeIds, staff, onDone }) {
  ...
  return (
    <>
      <ModalHead title={`Refer ${employeeIds.length} resume(s) to a staff member`} onClose={app.closeModal} />
      <ModalBody>
        <Field label="Assign to">
          <Select value={staffId} onChange={(e) => setStaffId(e.target.value)}>
            {staff.map((s) => (
              <option key={s.id} value={s.id}>
                {s.name} ({s.accessLevel === 'admin' ? 'Admin' : 'Worker'})
              </option>
            ))}
          </Select>
        </Field>
      ...
      <StaggerItem className="mb-4 flex items-center justify-between gap-2 flex-wrap">
        ...
        {selected.size > 0 && (
          <Button variant="primary" size="sm" onClick={() => openAssign([...selected])}>
            <UserPlus size={14} /> Refer {selected.size} selected
          </Button>
        )}
      </StaggerItem>
                ...
                <div className="flex items-start gap-4 flex-wrap">
                  ...
                  <div className="flex-1 min-w-[220px]">
                    ...
                    <div className="text-[12px] text-ink-tertiary mt-1">
                      {c.resume?.assignedTo?.name ? `Referred to ${c.resume.assignedTo.name}` : 'Not yet referred to a staff member'}
                    </div>
                    {c.skills?.length > 0 && (
                ...
                <div className="flex items-center gap-2 mt-4 pt-3.5 border-t border-border flex-wrap">
                  ...
                  {c.resume?.file && (
                    <Button size="sm" onClick={() => openAssign([c.id])}>
                      <UserPlus size={14} /> {c.resume?.assignedTo?.name ? 'Re-refer' : 'Refer'}
                    </Button>
                  )}
                </div>
              </Card>
