...
export default function Payments() {
  ...
  function recordCandidatePayment(p) {
    recordSubscription.mutate(p.id, {
      onSuccess: () => {
        app.addToast('success', `${fmtINR(p.amount)} recorded for ${p.party}`)
        refetch()
      },
  ...
  return (
    <StaggerGroup>
      <StaggerItem className="mb-6">
        ...
        <p className="text-sm text-ink-secondary mt-1">Two revenue lines: ₹299 one-time from each candidate, and ₹2,000 per opening from each employer.</p>
      </StaggerItem>
      ...
      <StaggerItem className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-5">
        ...
        <Card hover pad>
          ...
          <div className="text-[17px] font-bold tracking-tight mt-2.5">₹299 / candidate</div>
          <div className="text-[17px] font-bold tracking-tight">{fmtINR(2000)} / opening</div>
        </Card>
      </StaggerItem>
