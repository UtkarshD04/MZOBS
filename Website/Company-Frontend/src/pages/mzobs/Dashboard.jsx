...
const KPI_ORDER = ['candidates', 'resumeQueue', 'mockQueue']
const KPI_ICONS = { candidates: Users, resumeQueue: FileCheck, mockQueue: Video }
const KPI_LABELS = { candidates: 'Candidates', resumeQueue: 'Resumes to verify', mockQueue: 'Mocks scheduled' }
...
export default function Dashboard() {
  ...
  return (
    <StaggerGroup>
      ...
      <StaggerItem className="grid grid-cols-2 lg:grid-cols-3 gap-4 mb-4">
        {KPI_ORDER.filter((key) => kpis[key] !== undefined).map((key) => {
          ...
          return (
            <Card key={key} hover pad>
              ...
              <div className="text-xs text-ink-tertiary mt-1 leading-snug">{KPI_LABELS[key]}</div>
            </Card>
          )
        })}
      </StaggerItem>
