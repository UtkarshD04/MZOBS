...
function HowItWorks({ steps }) {
  return (
    <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4">
      {steps.map((s) => (
        <Card key={s.n} pad className="h-full">
          <span className="text-xs font-bold text-gold-strong tracking-wide">{s.n}</span>
          <h5 className="text-[13.5px] font-semibold text-ink mt-2">{s.title}</h5>
          <p className="text-[12.5px] text-ink-secondary mt-1 leading-relaxed">{s.desc}</p>
        </Card>
      ))}
    </div>
  )
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="grid lg:grid-cols-12 gap-6 items-stretch">
        <Card pad className="lg:col-span-7">
